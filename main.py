...
from model.hospitalDao import Hospital, listar_hospital, guardar, eliminar, editar, listar_por_id
...
def hospital_edit(id):
    ...
    if hospital is None:
        flash("No se encontró el hospital.", "warning")
        return redirect("/hospital")
...
def consultorio_guardar():
    ...
    resultado = guacon(consultorio)  # Llamar la función guardar
    #return redirect("/consultorio")
    # Verifica si el resultado no es None
    if resultado is None:
        flash("Error desconocido al guardar el consultorio.", "danger")
    else:
        flash(resultado["message"], resultado["status"])
    return redirect("/consultorio")
...
def consultorio_actualizar():
    ...
    id_hospital = request.form["hospital"]
    consultorio = Consultorio(nombre=nombre, id_hospital=id_hospital)
    resultado = edicon(consultorio, id)  # Llamar la función editar
    #return redirect("/consultorio")
    # Verifica si el resultado no es None
    if resultado is None:
        flash("Error desconocido al actualizar el consultorio.", "danger")
    else:
        flash(resultado["message"], resultado["status"])
    return redirect("/consultorio")
...
def consultorio_delete(id):
    resultado = elicon(id)  # Eliminar el consultorio
    #return redirect("/consultorio")
    # Verifica si el resultado no es None
    if resultado is None:
